...
area = st.selectbox("Area Type: ", options=np.sort(['Super built-up  Area', 'Built-up  Area', 'Plot  Area', 'Carpet  Area']))

loc = st.selectbox("Location: ", options = np.sort(['Electronic City Phase II', 'Chikka Tirupathi',
...
sizebhk = st.selectbox("Size: ", options=np.sort(['2 BHK', '4 Bedroom', '3 BHK', '3 Bedroom', '1 RK', '4 BHK',
       '1 BHK', '5 BHK', '11 BHK', '5 Bedroom', '9 BHK', '2 Bedroom',
       '6 BHK', '7 BHK', '6 Bedroom']))


soc = st.selectbox("Society: ",options = np.sort(society_values))


avail = st.selectbox("Availability: ", options = np.sort(['19-Dec', 'Ready To Move', '18-Nov', '17-Oct', '21-Dec', '19-Sep',
...
bathroom = st.select_slider("Number of Bathrooms: ",options=[1.,2., 3., 4.,5., 6.,7.,8., 9.])

balcony = st.select_slider("Number of balconies: ",options = [ 0.,1.,2.,3.])
...
set_background("bangalore_image3.jpg")
